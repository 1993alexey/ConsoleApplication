...
export class PlannerService {
  ...
  addPlanner(planner: Planner): Observable<Planner> {
    for (let i = 0; i < planner.talks.length; i++) {
      const talk = planner.talks[i]
      if (typeof talk.timeLimit == 'string')
        talk.timeLimit = parseInt(talk.timeLimit)
    }

    if (typeof planner.musicalNumber.performAfterTalk == 'string')
      planner.musicalNumber.performAfterTalk = parseInt(planner.musicalNumber.performAfterTalk)
